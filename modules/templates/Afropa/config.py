...
def config(settings):
    """
        Template settings for a Afropa system
    ...
    # settings.base.theme = "Afropa" #msw
    ...
    settings.L10n.languages = OrderedDict([  # msw
        ("de", "Deutsch"),
        ("en", "English"),
        ("fr", "Français"),
        ("ar", "العربية"),
    ...
    settings.L10n.religions = {"none": T("none"),  # msw
    ...
    # settings.ui.export_formats = ("kml", "pdf", "rss", "xls", "xml", "csv") # msw: added csv for testing DOES NOT WORK
    # settings.ui.export_formats = ("kml", "pdf") # msw: reducing works (after restart of web2py)
    ...
    settings.ui.default_cancel_button = True  # msw
    ...
    # settings.ui.menu_logo = URL(c="static", f="img", args=["S3menulogo.png"]) # original
    ...
    settings.pr.hide_third_gender = False  # msw
    ...
    settings.org.branches = True  # msw
    ...
    # settings.hrm.org_required = False # msw - uncommenting does not work out of the box?!
    ...
    settings.modules = OrderedDict([
        ...
        ("pr", Storage(
            ...
            restricted=True,
            access="|1|",
            # Only Administrators can see this module in the default menu (access to controller is possible to all still)
            module_type=10
        )),
        ("org", Storage(
    ...
    # e.g. change a field name and the help text
    def customise_org_organisation_controller(**attr):
        table = current.s3db.org_organisation
        table.year.label = T("Year Fooooounded")
        table.year.comment = DIV(_class="tooltip",
                                 _title="%s|%s" % (T("Year Founded"),
                                                   T("Year that the strange organization was founded."))),
        return attr

    settings.customise_org_organisation_controller = customise_org_organisation_controller

    # change another field name in a different table
    def customise_org_organisation_type_controller(**attr):
        table = current.s3db.org_organisation_type
        table.name.label = T("Name")
        return attr

    settings.customise_org_organisation_type_controller = customise_org_organisation_type_controller

    # hide a field
    def customise_org_office_resource(r, tablename):
        table = current.s3db.org_office
        table.phone2.readable = False
        table.phone2.writable = False
